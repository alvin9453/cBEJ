...
BejTuple_t *pack_json_to_sfv(const cJSON *json, EntryInfo_t *major_dict, EntryInfo_t *annotation_dict)
{
    ...
    if (json->type == cJSON_Object)
    {
        ...
        {
            BejTuple_t *packed_result = pack_json_to_sfv(obj, major_dict, annotation_dict);
    ...
    else if (json->type == cJSON_Array)
    {
        ...
        cJSON_ArrayForEach(obj, json)
        {
            memcpy(varray->tuples, pack_json_to_sfv(obj->child, major_dict, annotation_dict), sizeof(BejTuple_t *));
            varray->tuples++;
        }
        bej_V = varray;
    }
...
void encodeJsonToBinary(BejTuple_t *bej_tuple_list, cJSON *json_input, EntryInfo_t *major_dict, EntryInfo_t *annotation_dict)
{
    ...
    {
        memcpy(root_bej_V_init_ptr, pack_json_to_sfv(obj, major_dict, annotation_dict), sizeof(BejTuple_t *));
        root_bej_V_init_ptr++;
    }
